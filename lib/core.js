...
// TODO: Протестировать, описать методы.
...
class Core {
    ...
    async createClass(graphId, className, properties) {
        try {
            ...
            let classDescription = { name: className, properties: properties.map(p => { return { name: p.getName(), type: p.getType() }})}  // Contains the class description that will be returned to the user
            ...
            await session.writeTransaction(tx => {
                tx.run(`
                    ...
                    FOREACH (property IN $properties | 
                        CREATE (p:Property) 
                        SET p = { name: property.name, _unique_key: id(class) + "_" + property.name } 
                        MERGE (class)-[:HAVE]->(p))
