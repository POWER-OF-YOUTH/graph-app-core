...
class Core {
    ...
    async getGraph(graphId, extended = false) {
        try {
            ...
            if (extended) {
                let nodesDbResponse = await session.run(`
                ...
                if (nodesDbResponse.records.length !== 0)
                    graphDescription.nodes = nodesDbResponse.records.map(record => record.get("description"));

                let classesDbResponse = await session.run(`
                ...
                if (classesDbResponse.records.length !== 0)
                    graphDescription.classes = classesDbResponse.records.map(record => record.get("description"));
     ...
     * @param {String} graphId Graph id
     * @param {bool} extended Return extended graph information: nodes, classes and relationships
     * @returns {Promise<Array<{id: String, name: String, created: Date, nodes: Array<{id: String, values: Array<{name: String, type: String, value: any}>}>, classes: Array<{name: String, properties: Array<{name: String, type: String}>}>}>>}
     */
